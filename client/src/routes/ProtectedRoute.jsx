...
// ⚠️ This hides, it does not protect. Anyone can call the API directly and skip React
// entirely, so every rule this appears to enforce must also hold on the server.
// (Build rule 1)
export default function ProtectedRoute({ allow }) {
  ...
  if (allow) {
    ...
    // stored user, so deciding early sends a supervisor back to the landing page on
    // every refresh of their own screen.
    if (!sessionReady) {
      return (
    ...
    // `supervisor` is derived and never in `roles`: it is answered by the server's
    // reading of who leads a unit today.
